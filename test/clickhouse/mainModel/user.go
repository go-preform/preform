...
func (f FactoryUser) CloneInstance(factory preform.IFactory) preform.IFactory {
	...
	ff.ManagerIds = cols[2].(*preform.ForeignKey[[]int32] )
	ff.CreatedBy = cols[3].(*preform.ForeignKey[int32] )
	ff.CreatedAt = cols[4].(*preform.PrimaryKey[time.Time] )
	ff.LoginedAt = cols[5].(*preform.Column[*time.Time] )
	ff.DetailAge = cols[6].(*preform.Column[[]int32] )
	ff.DetailDateOfBirth = cols[7].(*preform.Column[[]time.Time] )
...
func (m UserBody) FieldValueImmutablePtrs() []any { return []any{&m.Id, &m.Name, &m.ManagerIds, &m.CreatedBy, &m.CreatedAt, &m.LoginedAt, &m.DetailAge, &m.DetailDateOfBirth} }
...
func (m *UserBody) FieldValuePtr(pos int) any { 
	switch pos {
		...
		case 6: return &m.DetailAge
		case 7: return &m.DetailDateOfBirth
	}
	return nil
}

func (m *UserBody) FieldValuePtrs() []any { 
	return []any{&m.Id, &m.Name, &m.ManagerIds, &m.CreatedBy, &m.CreatedAt, &m.LoginedAt, &m.DetailAge, &m.DetailDateOfBirth}
}
