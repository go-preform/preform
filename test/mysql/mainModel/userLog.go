...
func (f FactoryUserLog) CloneInstance(factory preform.IFactory) preform.IFactory {
	...
	ff.RelatedLogId = cols[1].(*preform.ForeignKey[preformTypes.Null[int64]] )
	ff.Type = cols[2].(*preform.Column[PreformTestAUserLogType] )
	ff.UserId = cols[3].(*preform.ForeignKey[int32] )
...
func (m UserLogBody) FieldValueImmutablePtrs() []any { return []any{&m.Id, &m.RelatedLogId, &m.Type, &m.UserId} }
...
func (m *UserLogBody) FieldValuePtrs() []any { 
	return []any{&m.Id, &m.RelatedLogId, &m.Type, &m.UserId}
}
