...
func (f FactoryUserLog) CloneInstance(factory preform.IFactory) preform.IFactory {
	...
	ff.RelatedLogId = cols[1].(*preform.ForeignKey[preformTypes.Null[uuid.UUID]] )
	ff.Type = cols[2].(*preform.Column[PreformTestALogType] )
	ff.Detail = cols[3].(*preform.Column[PreformTestALogDetail] )
	ff.UserId = cols[4].(*preform.ForeignKey[int32] )
...
func (m UserLogBody) FieldValueImmutablePtrs() []any { return []any{&m.Id, &m.RelatedLogId, &m.Type, &m.Detail, &m.UserId} }

func (m *UserLogBody) FieldValuePtr(pos int) any { 
	switch pos {
		case 0: return &m.Id
		case 1: return &m.RelatedLogId
		case 2: return &m.Type
		case 3: return &m.Detail
		case 4: return &m.UserId
	}
	return nil
}

func (m *UserLogBody) FieldValuePtrs() []any { 
	return []any{&m.Id, &m.RelatedLogId, &m.Type, &m.Detail, &m.UserId}
}

func (m *UserLogBody) RelatedValuePtrs() []any { return []any{&m.UserLogByUserLogUserLogFk, &m.UserByUserLogUserFk, &m.UserByUserLogUserFkRegister, &m.UserLogsByUserLogUserLogFk} }


func (m *UserLogBody) RelatedByPos(pos uint32) any {
	switch pos {
			case 0: return &m.UserLogByUserLogUserLogFk
			case 1: return &m.UserByUserLogUserFk
			case 2: return &m.UserByUserLogUserFkRegister
