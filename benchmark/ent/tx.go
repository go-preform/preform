...
func (tx *Tx) init() {
	tx.TestA = NewTestAClient(tx.config)
	tx.TestB = NewTestBClient(tx.config)
	tx.TestC = NewTestCClient(tx.config)
}
...
// applies a query, for example: TestA.QueryXXX(), the query will be executed
