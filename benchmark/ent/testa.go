...
// TestBsOrErr returns the TestBs value or an error if the edge
// was not loaded in eager-loading.
func (e TestAEdges) TestBsOrErr() ([]*TestB, error) {
	if e.loadedTypes[0] {
		return e.TestBs, nil
	}
	return nil, &NotLoadedError{edge: "test_bs"}
}

// scanValues returns the types for scanning values from sql.Rows.
func (*TestA) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case testa.FieldBool:
			values[i] = new(sql.NullBool)
		case testa.FieldFloat4, testa.FieldFloat8:
			values[i] = new(sql.NullFloat64)
		case testa.FieldID, testa.FieldInt4, testa.FieldInt8:
			values[i] = new(sql.NullInt64)
		case testa.FieldName, testa.FieldText:
...
// to the TestA fields.
func (t *TestA) assignValues(columns []string, values []any) error {
...
// Value returns the ent.Value that was dynamically selected and assigned to the TestA.
...
func (t *TestA) QueryTestBs() *TestBQuery {
	return NewTestAClient(t.config).QueryTestBs(t)
}

// Update returns a builder for updating this TestA.
// Note that you need to call TestA.Unwrap() before calling this method if this TestA
// was returned from a transaction, and the transaction was committed or rolled back.
func (t *TestA) Update() *TestAUpdateOne {
	return NewTestAClient(t.config).UpdateOne(t)
}

// Unwrap unwraps the TestA entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (t *TestA) Unwrap() *TestA {
	_tx, ok := t.config.driver.(*txDriver)
	if !ok {
		panic("ent: TestA is not a transactional entity")
	}
